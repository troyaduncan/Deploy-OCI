...
# Normalize app name for container/image identifiers (OCI requires lowercase repository names)
APP_RAW="$APP"
APP_NORM="$(echo "$APP_RAW" | tr '[:upper:]' '[:lower:]')"
# Replace any unsupported characters with hyphen for image/container safety
APP_NORM="$(echo "$APP_NORM" | sed 's/[^a-z0-9._-]/-/g')"
if [[ "$APP_RAW" != "$APP_NORM" ]]; then
  APP_NAME_NOTE="NOTE: app name normalized for image/container: \"$APP_RAW\" -> \"$APP_NORM\""
else
  APP_NAME_NOTE=""
fi
...
remote_have_rsync() {
  # rsync requires the binary on BOTH ends
  if [[ "$DRY_RUN" == "true" ]]; then
    return 0
  fi
  ssh "${SSH_OPTS[@]}" "${REMOTE_USER}@${HOST}" "command -v rsync >/dev/null 2>&1"
}
...
transfer_file() {
  ...
  if [[ "$TRANSFER" == "rsync" ]]; then
    local local_ok="no"
    local remote_ok="no"
    if have_rsync; then local_ok="yes"; fi
    if remote_have_rsync; then remote_ok="yes"; fi

    if [[ "$local_ok" == "yes" && "$remote_ok" == "yes" ]]; then
      rsync -avP --inplace -e "ssh ${SSH_OPTS[*]}"         "$local_path" "${REMOTE_USER}@${HOST}:${remote_dir}/"
      return 0
    fi

    echo "WARN: rsync requested but unavailable (local: ${local_ok}, remote: ${remote_ok}). Falling back to scp."
  fi

  # scp uses -P (uppercase) for port, not -p; build scp-specific opts
  local SCP_OPTS=(
    -P "$SSH_PORT"
    -o "ServerAliveInterval=${SSH_KEEPALIVE}"
    -o "ServerAliveCountMax=${SSH_KEEPALIVE_COUNT}"
  ...
  scp "${SCP_OPTS[@]}" "$local_path" "${REMOTE_USER}@${HOST}:${remote_dir}/"
}
...
create_default_containerfile_if_missing() {
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    echo "[dry-run] create default Containerfile"
    return 0
  fi
  ...
  local entrypoint_cmd='["npm", "start"]'
  if [[ -f "${app_dir}/server.js" ]]; then
    entrypoint_cmd='["node", "server.js"]'
  elif [[ -f "${app_dir}/index.js" ]]; then
    entrypoint_cmd='["node", "index.js"]'
  elif [[ -f "${app_dir}/dist/index.js" ]]; then
    entrypoint_cmd='["node", "dist/index.js"]'
  elif [[ -f "${app_dir}/dist/index.cjs" ]]; then
    entrypoint_cmd='["node", "dist/index.cjs"]'
  fi

  # ---- Detect CA bundle for corporate/air-gapped environments ----
  local has_ca="false"
  local ca_source=""
  if [[ -f "${app_dir}/deploy/ca-certificates.crt" ]]; then
    has_ca="true"
  elif [[ -f "/etc/ssl/certs/ca-certificates.crt" ]]; then
    # Auto-copy system CA bundle into the project
    mkdir -p "${app_dir}/deploy"
    cp /etc/ssl/certs/ca-certificates.crt "${app_dir}/deploy/"
    has_ca="true"
    ca_source=" (copied from system)"
  fi

  # ---- Generate Containerfile ----
  {
    echo "# Default Containerfile generated by deploy-oci.sh"
    echo "FROM node:20-slim"

    if [[ "$has_ca" == "true" ]]; then
      echo ""
      echo "# Inject corporate CA bundle (required for air-gapped / TLS-intercepting environments)"
      echo "COPY deploy/ca-certificates.crt /etc/ssl/certs/ca-certificates.crt"
      echo "ENV SSL_CERT_FILE=/etc/ssl/certs/ca-certificates.crt"
      echo "ENV NODE_EXTRA_CA_CERTS=/etc/ssl/certs/ca-certificates.crt"
    fi
    ...
    case "$pkg_mgr" in
      pnpm)
        ...
        if [[ -f "${app_dir}/pnpm-workspace.yaml" ]]; then
          echo "COPY package.json pnpm-lock.yaml pnpm-workspace.yaml ./"
          # Copy .npmrc if present (may contain shamefully-hoist, registries, etc.)
          if [[ -f "${app_dir}/.npmrc" ]]; then
            echo "COPY .npmrc ./"
          fi
          # Auto-detect workspace package.json files
          for pkg_json in $(find "${app_dir}" -maxdepth 3 -name "package.json" \
              -not -path "*/node_modules/*" -not -path "${app_dir}/package.json" \
              | sort); do
            local rel_dir="${pkg_json#${app_dir}/}"
            rel_dir="${rel_dir%/package.json}"
            echo "COPY ${rel_dir}/package.json ${rel_dir}/"
          done
        else
      ...
      yarn)
        ...
        echo "RUN yarn install --frozen-lockfile --production"
        ;;
      npm)
        echo "# Install dependencies"
        echo "COPY package*.json ./"
        echo "RUN npm ci --omit=dev --no-audit --no-fund"
        ;;
    esac
    ...
    echo "EXPOSE ${PORT_CONT}"
    echo ""
    echo "CMD ${entrypoint_cmd}"
  } > "$cf"

  echo "==> Created default Containerfile (base: node:20-slim, pkg: ${pkg_mgr}, port: ${PORT_CONT})"
  if [[ "$has_ca" == "true" ]]; then
    echo "    CA bundle injected${ca_source}"
  fi
  echo "    Review CMD/EXPOSE if needed: $cf"

  # ---- Create .dockerignore if missing ----
  if [[ ! -f "${app_dir}/.dockerignore" && ! -f "${app_dir}/.containerignore" ]]; then
    cat > "${app_dir}/.dockerignore" <<'DIEOF'
node_modules
.git
.cache
*.tsbuildinfo
...
CONTAINER_NAME="$APP_NORM"
PORT_HOST="${PORT_MAP%%:*}"
PORT_CONT="${PORT_MAP##*:}"

echo "==> Deployment plan"
print_kv "Local app dir:" "$LOCAL_APP_DIR"
print_kv "App name (raw):" "$APP_RAW"
print_kv "App name (normalized):" "$APP_NORM"
if [[ -n "${APP_NAME_NOTE}" ]]; then
  echo "==> WARNING: ${APP_NAME_NOTE}"
fi
...
BUILD_ARGS=(-f "$BUILD_FILE" -t "$IMAGE")
if [[ "$ENGINE" == "podman" ]]; then
  BUILD_ARGS+=(--network=host)
fi
run_local "$ENGINE" build "${BUILD_ARGS[@]}" .
...
if [[ "$DRY_RUN" != "true" ]]; then
  (cd "$(dirname "$LOCAL_ARCHIVE")" && sha256sum "$(basename "$LOCAL_ARCHIVE")") > "$LOCAL_SHA"
...
# Stop systemd unit first if it exists (prevents race where systemd restarts
# the container between rm -f and the new podman run)
if [[ "$USE_SYSTEMD" == "true" && "$DRY_RUN" != "true" ]]; then
  SYSTEMD_UNIT="container-${CONTAINER_NAME}.service"
  if [[ "$EFFECTIVE_SCOPE" == "user" ]]; then
    ssh "${SSH_OPTS[@]}" "${REMOTE_USER}@${HOST}" \
      "systemctl --user stop '$SYSTEMD_UNIT' 2>/dev/null; systemctl --user disable '$SYSTEMD_UNIT' 2>/dev/null" || true
  else
    ssh "${SSH_OPTS[@]}" "${REMOTE_USER}@${HOST}" \
      "sudo systemctl stop '$SYSTEMD_UNIT' 2>/dev/null; sudo systemctl disable '$SYSTEMD_UNIT' 2>/dev/null" || true
  fi
fi
...
if [[ "$KEEP_IMAGES" != "0" ]]; then
  echo "==> Pruning remote images (best-effort, keep newest ${KEEP_IMAGES} for repo local/${APP_NORM})..."
  ssh_run "
    set -euo pipefail
